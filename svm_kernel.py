...
class RBFKernel:
    ...
    def eval(self, Z, s):
        return np.exp(
            -((self.X[s, np.newaxis, :] - Z[np.newaxis, :, :]) ** 2).sum(axis=2)
            / (2 * self.sigma2)
        )
...
class SVC:
    ...
    def predict(self, X):
        s = self.a_ != 0.0
        return np.sign(
            self.w0_ + np.dot(self.a_[s] * self.y_[s], self.kernel_.eval(X, s))
        )
